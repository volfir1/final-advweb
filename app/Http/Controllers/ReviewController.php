<?php
    ...
    class ReviewController extends Controller
    {
        ...
        public function fetchCompletedOrdersForReview(Request $request)
        {
            try {
                $user = Auth::user();
        
                // Fetch completed orders and their products for the authenticated user
                $orders = Order::with(['products', 'products.reviews'])
                    ->where('customer_id', $user->id)
                    ->where('status', 'completed')
                    ->get();
                ...
                foreach ($orders as $order) {
                    ...
                    foreach ($order->products as $product) {
                        if ($product->reviews->where('customer_id', $user->id)->isNotEmpty()) {
                            $hasReview = true;
                            break;
                        }
                    }
                ...
                Log::error('Error in ReviewController@fetchCompletedOrdersForReview: ' . $e->getMessage());
